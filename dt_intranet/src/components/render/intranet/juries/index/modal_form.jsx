...
import './../../../../../sass/render/intranet/juries/index/modal_form.sass'
import {
    Input,Preloader
...
class ModalForm extends Component{
    ...
    componentDidUpdate(){
        ...
        // };
        this.evaluateStated( this.props.jury.name,$(".modal_juries #name~label") ) 
        this.evaluateStated( this.props.jury.last_name,$(".modal_juries #last_name~label") )
        this.fetchSchools()
    ...
    render(){
        return(
            <div id={this.props.id} className="modal modal_juries">
                    <div className="modal-content">
                        <h4 className="type">
                            {
                                this.props.type==="create"?
                                                    ...
                                                    :((this.props.type)==="update"?
                                                        "Editar Jurado"    
                                                    :   "¿Seguro que desea eliminar este Jurado?")
                                                }
                        </h4>
                        <form action="" className="content_inputs" method="POST<"
                                ...
                                <Input id="last_name"
                                        ...
                                        type ="text"
                                        label="Apellido(s)"
                                        value={this.props.jury.last_name||""}
    ...
    onAction(e){//  ... create || update || delete
        ...
        formData.append("name",this.props.jury.name.toUpperCase())
        formData.append("last_name", this.props.jury.last_name.toUpperCase())
        if (this.props.jury.name&&this.props.jury.last_name) {
            if(this.props.type!=="create") formData.append("id",this.props.jury.id)
            $(this.preloader).addClass("show-preloader")
            fetch(global.URLBASESERVICE+`/jury/${this.props.type}`,{
                credentials : "include",
                method : "POST",
                body: formData
            })
            ...
            .catch( reject=>{
                Materialize.toast("El jurado esta relacionado con una tesis",2000)
                $(this.preloader).removeClass("show-preloader")
            })
        }else{
